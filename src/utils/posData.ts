...
export function generateSampleTransactions(
  ...
  const paymentMethods: Array<'cash' | 'card' | 'mobile' | 'check'> = [
    'cash',
    'card',
    'mobile',
    'check',
 ...
 */
export function formatTransactionsForAnalysis(transactions: POSTransaction[]): string {
  ...
  let totalTax = 0;
  let totalDiscount = 0;
  const productStats: Record<string, { count: number; revenue: number; qty: number }> = {};
 ...
 */
export function generateCategoryStats(transactions: POSTransaction[]): Record<string, any> {
...
export function comparePeriods(
  period1: POSTransaction[],
  period2: POSTransaction[],
): Record<string, any> {
  const stats1 = {
