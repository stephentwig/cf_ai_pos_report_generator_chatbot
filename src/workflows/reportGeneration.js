...
import { generateSampleTransactions, formatTransactionsForAnalysis, generateCategoryStats, } from '../utils/posData';
/**
 * Execute report generation workflow
 */
export async function executeReportWorkflow(env, context) {
    try {
        ...
        const llmResponse = await generateReport(env, context.reportType, formattedData, context.filters);
        if (!llmResponse.content) {
            throw new Error('Empty response from LLM');
        }
        // Step 4: Process and structure report
        console.log('Step 4: Processing report...');
        const report = await processReportResponse(context, llmResponse.content, posData.stats);
        // Step 5: Cache report
 ...
 */
async function fetchPOSData(_env, filters) {
    ...
    const transactions = generateSampleTransactions(filters.startDate, filters.endDate, transactionCount);
    // Apply filters
    let filtered = transactions;
    if (filters.department || filters.category) {
        filtered = filtered.filter((txn) => txn.items.some((item) => {
 ...
 */
async function processReportResponse(context, response, _stats) {
 ...
 */
async function cacheReport(env, report) {
    try {
 ...
 */
export async function getCachedReport(env, reportId) {
    try {
        const kv = env.REPORTS_CACHE;
        if (!kv)
            return null;
